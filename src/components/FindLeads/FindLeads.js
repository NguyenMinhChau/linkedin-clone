...
import styles from './FindLeads.module.css';
import { EmptyState } from 'evergreen-ui';

function FindLeads() {
    useEffect(() => {
        document.title = 'FindLeads | Linkedln';
    }, []);
    return (
        <div className={`${clsx(styles.find_leads)}`}>
            <EmptyState
                className={`${clsx(styles.emptystate)}`}
                background='dark'
                title='The page FIND LEADS is developing interface...'
