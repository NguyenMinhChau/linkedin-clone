...
import styles from './GuestControls.module.css';
import { EmptyState } from 'evergreen-ui';

function GuestControls() {
    useEffect(() => {
        document.title = 'Guest Controls | Linkedln';
    }, []);
    return (
        <div className={`${clsx(styles.guest_controls)}`}>
            <EmptyState
                className={`${clsx(styles.emptystate)}`}
                background='dark'
                title='The page GUEST CONTROLS is developing interface...'
